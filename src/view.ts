...
class NewsListTree extends vscode.TreeItem {
  ...
  constructor(label: string | vscode.TreeItemLabel) {
    ...
    newsList.forEach((value, index) => {
      if (
        value.c2c
          ? Global.client.pickFriend(value.uin)
          : Global.client.pickGroup(value.uin)
      ) {
...
class ContactsListTree extends vscode.TreeItem {
  ...
  constructor(label: string | vscode.TreeItemLabel) {
    super(label, vscode.TreeItemCollapsibleState.Expanded);
    this.friendClassesListTree = new ClassesListTree(
      '分组',
      Global.client.classes
    );
    this.groupClassesListTree = new ClassesListTree(
      '群组',
      new Map([
        [-1, '我创建的'],
        [-2, '我管理的'],
    ...
    if (classId >= 0) {
      ...
      for (let [, friend] of Global.client.fl) {
        if (friend.class_id === classId) {
          this.infoList.push(
            new InfoTreeItem(friend.remark, friend.user_id, true)
          );
        }
      }
    } else {
      ...
      for (let [, group] of Global.client.gl) {
        if (
          (this.label === '我创建的' && group.owner_id === Global.client.uin) ||
          (this.label === '我管理的' &&
            group.owner_id !== Global.client.uin &&
            group.admin_flag) ||
          (this.label === '我加入的' &&
            group.owner_id !== Global.client.uin &&
            !group.admin_flag)
          ...
          this.infoList.push(
            new InfoTreeItem(group.group_name, group.group_id, false)
          );
        }
      }
    }
...
class InfoTreeItem extends vscode.TreeItem {
  ...
  constructor(
    ...
    this.iconPath = vscode.Uri.parse(
      c2c
        ? Global.client.pickFriend(uid).getAvatarUrl(40)
...
function createTreeView() {
  ...
  newsList = [];
  Global.qliteTreeView = vscode.window.createTreeView('contactView', {
  ...
  Global.client.on('notice.group.transfer', (event) => {
    if (event.user_id === Global.client.uin) {
      vscode.window.showInformationMessage(
        '群：' + event.group.name + ' 的群主已将群主身份转让给你'
      );
    }
  });
  ...
  Global.client.on('notice.group.decrease', (event) => {
    if (event.user_id === Global.client.uin) {
      ...
      if (event.dismiss) {
        ...
        msg =
          event.group.pickMember(event.operator_id).card +
          ' 将你踢出了群:' +
          event.group.name;
      }
...
function refreshContact(c2c: boolean, uin: number, flag: boolean) {
  ...
  if (!news) {
    ...
    const label = c2c
      ...
      : Global.client.pickGroup(uin).name;
    // 初始化新消息
    news = {
      c2c: c2c,
      uin: uin,
      item: new InfoTreeItem(
        label as string,
        uin,
        c2c,
        flag ? '+1' : '',
